(function (exports) {
  ...
  function getEquivalents(q, r, dir) {
    ...
    if (dir === 0) {
      equivalents.push({ q: q + 1, r: r - 1, d: 2 }, { q: q + 1, r: r, d: 4 });
    } else if (dir === 1) {
      equivalents.push({ q: q + 1, r: r, d: 3 }, { q: q, r: r + 1, d: 5 });
    } else if (dir === 2) {
      equivalents.push({ q: q, r: r + 1, d: 4 }, { q: q - 1, r: r + 1, d: 0 });
    } else if (dir === 3) {
      equivalents.push({ q: q - 1, r: r + 1, d: 5 }, { q: q - 1, r: r, d: 1 });
    } else if (dir === 4) {
      equivalents.push({ q: q - 1, r: r, d: 0 }, { q: q, r: r - 1, d: 2 });
    } else if (dir === 5) {
      equivalents.push({ q: q, r: r - 1, d: 1 }, { q: q + 1, r: r - 1, d: 3 });
    }
    return equivalents;
  }
  ...
  exports.hasConnectedInfrastructure = function (edgeId, playerId, gameState) {
    ...
    const hasBuilding = vertices.some((vId) =>
      gameState.buildings.some(
        (b) => b.vertexId === vId && b.owner === playerId
      )
    );
  ...
  exports.getPlayerTradeRatio = function (playerId, resourceType, gameState) {
    ...
    for (let b of playerBuildings) {
      const port = gameState.ports.find((p) =>
        p.vertexIds.includes(b.vertexId)
      );
      if (port) {
        if (port.type === "3:1") {
          if (ratio > 3) ratio = 3;
