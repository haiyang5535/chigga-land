...
function isPlayersTurn(socketId) {
    if (gameState.phase !== 'MAIN_GAME' && gameState.phase !== 'SETUP_ROUND_1' && gameState.phase !== 'SETUP_ROUND_2') return false;
...
function initBoard() {
    ...
    // q, r are axial coordinates for hex grids
    // This is a very small map example to start
    for (let q = -2; q <= 2; q++) {
        for (let r = -2; r <= 2; r++) {
            if (Math.abs(q + r) <= 2) {
    ...
    const portTypes = ['3:1', 'wood', 'brick', '3:1', 'sheep', 'wheat', '3:1', 'ore', '3:1'];
    let portIndex = 0;
...
io.on('connection', (socket) => {
    ...
    if (player) {
        ...
        console.log(`Player reconnected: ${player.name} (${oldSocketId} -> ${socket.id})`);
        ...
        gameState.buildings.forEach(b => { if(b.owner === oldSocketId) b.owner = socket.id; });
        gameState.roads.forEach(r => { if(r.owner === oldSocketId) r.owner = socket.id; });
        gameState.activeOffers.forEach(o => { if(o.from === oldSocketId) o.from = socket.id; });
        if (gameState.largestArmy.owner === oldSocketId) gameState.largestArmy.owner = socket.id;
        if (gameState.longestRoad.owner === oldSocketId) gameState.longestRoad.owner = socket.id;
        if (gameState.pendingDiscards.includes(oldSocketId)) {
            gameState.pendingDiscards = gameState.pendingDiscards.map(id => id === oldSocketId ? socket.id : id);
        }
    ...
    socket.on('startGame', () => {
        ...
        if (playerCount < 1) { // Reduced to 1 for testing
            socket.emit('logMessage', "Not enough players to start (need at least 1).");
            return;
        }
        ...
        io.emit('logMessage', `It is ${gameState.players[gameState.turnOrder[0]].name}'s turn.`);
    });
    ...
    function handleEndTurn() {
        ...
        io.emit('logMessage', `It is ${gameState.players[nextPlayerId].name}'s turn.`);
    }

    socket.on('endTurn', () => {
        if (!isPlayersTurn(socket.id)) return;
        if (gameState.phase === 'MAIN_GAME' && !gameState.hasRolled) {
            socket.emit('logMessage', "You must roll the dice before ending your turn.");
            return;
        }
    ...
    socket.on('buildNode', (data) => {
        ...
        if (!isSetup && !Shared.canAfford(player.resources, Shared.COSTS.settlement)) {
        ...
        if (isSetup) {
            ...
            if (gameState.phase === 'SETUP_ROUND_2') {
                 ...
                 // This is complex to calculate without hex lookup, skipping for now or implementing simply
        ...
        if (isSetup && gameState.setupItemsPlaced.settlement && gameState.setupItemsPlaced.road) {
            handleEndTurn();
        }
    });
    ...
    socket.on('bankTrade', (data) => {
        ...
        io.emit('logMessage', `${player.name} traded ${ratio} ${data.give} for 1 ${data.get} with the bank.`);
        io.emit('playerUpdate', gameState.players);
    });
    ...
    socket.on('discardResources', (data) => {
        ...
        gameState.pendingDiscards = gameState.pendingDiscards.filter(id => id !== socket.id);
        io.emit('logMessage', `${player.name} discarded resources.`);
        
        if (gameState.pendingDiscards.length === 0) {
            gameState.phase = 'ROBBER_PLACEMENT';
            io.emit('logMessage', "All discards complete. Move the Robber!");
            io.emit('init', gameState);
        }
        io.emit('playerUpdate', gameState.players);
    });
    ...
    socket.on('rollDice', () => {
        ...
        if (roll === 7) {
            io.emit('logMessage', "ROBBER ACTIVATED! Players with > 7 cards must discard half.");
            ...
            if (gameState.pendingDiscards.length > 0) {
                ...
                gameState.phase = 'ROBBER_PLACEMENT';
                io.emit('logMessage', "Move the Robber!");
            }
        ...
        const matchingHexes = gameState.board.filter(h => h.number === roll && h.resource !== 'desert' && (h.q !== gameState.robber.q || h.r !== gameState.robber.r));
            ...
            for (let i = 0; i < 6; i++) {
                ...
                if (building) {
                    ...
                    if (owner) {
                        ...
                        owner.resources[hex.resource] = (owner.resources[hex.resource] || 0) + amount;
                        io.emit('logMessage', `${owner.name} received ${amount} ${hex.resource}`);
                    }
                }
            }
        });
    ...
    socket.on('acceptTrade', (data) => {
        ...
        const offerIndex = gameState.activeOffers.findIndex(o => o.id === offerId);
    ...
    socket.on('cancelOffer', (data) => {
        const offerIndex = gameState.activeOffers.findIndex(o => o.id === data.offerId);
        if (offerIndex !== -1 && gameState.activeOffers[offerIndex].from === socket.id) {
            gameState.activeOffers.splice(offerIndex, 1);
            io.emit('init', gameState);
        }
    });
    ...
    socket.on('playDevCard', (data) => {
        ...
        const cardIndex = player.devCards.findIndex(c => c.type === data.type && c.boughtTurn !== gameState.turnCount);
        
        if (cardIndex === -1) {
            socket.emit('logMessage', "Cannot play this card (bought this turn or don't have it).");
            return;
        }
        ...
        if (data.type === 'knight') {
            ...
            io.emit('logMessage', `${player.name} played Monopoly on ${res} and got ${totalStolen}!`);
        } else if (data.type === 'roadBuilding') {
            player.devCards.splice(cardIndex, 1);
            player.freeRoads = 2;
            io.emit('logMessage', `${player.name} played Road Building (2 free roads).`);
        }
    ...
    socket.on('moveRobber', (data) => {
        ...
        gameState.robber = { q: data.q, r: data.r };
        gameState.phase = 'MAIN_GAME';
        ...
        if (victims.length > 0) {
            ...
            if (available.length > 0) {
                ...
                io.emit('logMessage', `${thief.name} tried to steal from ${victim.name} but they had nothing!`);
            }
        }
...
function updateLongestRoads() {
    ...
    const lengths = {};
    for (let pid in gameState.players) {
        lengths[pid] = Shared.calculateLongestRoad(pid, gameState.roads, gameState.buildings);
    }
    ...
    if (bestPid) {
        ...
        if (bestPid !== currentOwner && bestLen > currentLen) {
            ...
            io.emit('logMessage', `${gameState.players[bestPid].name} took Longest Road (${bestLen})!`);
...
function updateLargestArmy() {
    ...
    if (maxPid) {
        ...
        if (maxPid !== currentOwner && maxArmy > currentSize) {
            ...
            io.emit('logMessage', `${gameState.players[maxPid].name} took Largest Army (${maxArmy})!`);
